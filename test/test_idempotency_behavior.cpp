...
FSM_STATE_MACHINE(IdempotencyMachine, IParent) { int handled_calls = 0; };

FSM_STATE(IParent, IdempotencyMachine, ILeaf) { TRACK_LIFECYCLE(IParent); };

FSM_STATE(ILeaf, IParent) {
  using reactions = ufsm::List<ufsm::Reaction<IEventHandled> >;
