...
FSM_STATE_MACHINE(UnhandledHookMachine, UHTop) {
  ...
  void OnUnhandledEvent(const ufsm::detail::EventBase&) { unhandled_count++; }
};

FSM_STATE(UHTop, UnhandledHookMachine, UHLeaf){};

FSM_STATE(UHLeaf, UHTop) {
  using reactions = ufsm::List<ufsm::Reaction<UHEvHandled> >;

  ufsm::Result React(const UHEvHandled&) { return ConsumeEvent(); }
};
