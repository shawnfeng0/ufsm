...
FSM_STATE(DTTop, DeclarativeTransitionMachine, DTStateA) { TRACK_LIFECYCLE(DTTop); };

// DTEventToB triggers a direct state transition without defining React().
FSM_STATE(DTStateA, DTTop) {
  using reactions = ufsm::List<ufsm::Transition<DTEventToB, DTStateB> >;

  TRACK_LIFECYCLE(DTStateA);
};

FSM_STATE(DTStateB, DTTop) { TRACK_LIFECYCLE(DTStateB); };

// Action-on-LCA coverage.
FSM_STATE_MACHINE(DeclarativeActionSiblingMachine, DTParent) { int action_counter = 0; };

FSM_STATE(DTParent, DeclarativeActionSiblingMachine, DTLeafA) { TRACK_LIFECYCLE(DTParent); };

struct DTSiblingAction {
  void operator()(DTParent& parent) const { parent.OutermostContext().action_counter++; }
};

FSM_STATE(DTLeafA, DTParent) {
  using reactions = ufsm::List<ufsm::Transition<DTEventToSiblingWithAction, DTLeafB, DTSiblingAction> >;

  TRACK_LIFECYCLE(DTLeafA);
};

FSM_STATE(DTLeafB, DTParent) { TRACK_LIFECYCLE(DTLeafB); };

FSM_STATE_MACHINE(DeclarativeActionTopLevelMachine, DTTop1) { int action_counter = 0; };

FSM_STATE(DTTop1, DeclarativeActionTopLevelMachine, DTTop1Leaf) { TRACK_LIFECYCLE(DTTop1); };

struct DTTopLevelAction {
  void operator()(DeclarativeActionTopLevelMachine& machine) const { machine.action_counter++; }
};

FSM_STATE(DTTop1Leaf, DTTop1) {
  using reactions = ufsm::List<ufsm::Transition<DTEventToTop2WithAction, DTTop2, DTTopLevelAction> >;

  TRACK_LIFECYCLE(DTTop1Leaf);
};

FSM_STATE(DTTop2, DeclarativeActionTopLevelMachine) { TRACK_LIFECYCLE(DTTop2); };
