...
FSM_STATE_MACHINE(DestructorMachine, DStateA) {
    int counter = 0;
};

// DStateA -> DStateAA -> DStateAAA -> DStateAAAA
FSM_STATE(DStateA, DestructorMachine, DStateAA) {
    TRACK_LIFECYCLE(DStateA);
};

FSM_STATE(DStateAA, DStateA, DStateAAA) {
    TRACK_LIFECYCLE(DStateAA);
};

FSM_STATE(DStateAAA, DStateAA, DStateAAAA) {
    TRACK_LIFECYCLE(DStateAAA);
};
...
// DStateB -> DStateBA
FSM_STATE(DStateB, DestructorMachine, DStateBA) {
    TRACK_LIFECYCLE(DStateB);
};

FSM_STATE(DStateBA, DStateB) {
    TRACK_LIFECYCLE(DStateBA);
};
