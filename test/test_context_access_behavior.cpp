...
FSM_STATE(CtxParent, ContextAccessMachine, CtxLeafA) { int value = 0; };

FSM_STATE(CtxLeafA, CtxParent) {
  using reactions = ufsm::List<ufsm::Reaction<CtxEventToB> >;
  ...
  ufsm::Result React(const CtxEventToB&) {
    ...
    (void)&Context<CtxParent>();
    OutermostContext().parent_ptr_non_null_in_a = true;
...
FSM_STATE(CtxLeafB, CtxParent) {
  using reactions = ufsm::List<ufsm::Reaction<CtxEventPingB>, ufsm::Reaction<CtxEventToTop2> >;

  ufsm::Result React(const CtxEventPingB&) {
    OutermostContext().ping_b_calls++;
    (void)&Context<CtxParent>();
    OutermostContext().parent_ptr_non_null_in_ping_b = true;
    return ConsumeEvent();
  }
  ...
  ufsm::Result React(const CtxEventToTop2&) {
    ...
    (void)&Context<CtxParent>();
    OutermostContext().parent_ptr_non_null_in_to_top2 = true;
