...
class State : public detail::StateBaseType<MostDerived,
 ...
 public:
  ...
  detail::ReactionResult ReactImpl(const detail::EventBase& evt) override {
    ...
    if constexpr (std::is_same_v<reactions, detail::AutoReactionsTag>) {
        ...
        reaction_result = LocalReact<AutoGeneratedReactions>(evt);
    } else {
        reaction_result = LocalReact<reactions>(evt);
    }
