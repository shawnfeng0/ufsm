...
#include <ufsm/detail/auto_react_c17.h>
...
class State : public detail::StateBaseType<MostDerived, InnerInitial> {
 ...
 public:
  ...
  using reactions = detail::AutoReactionsTag;
 ...
 public:
  ...
  detail::ReactionResult ReactImpl(const detail::EventBase& evt) override {
    ...
    if constexpr (std::is_same<reactions, detail::AutoReactionsTag>::value) {
      using EventList = typename detail::GetEventList<MostDerived, OutermostContextType>::type;
      using AutoGeneratedReactions = typename detail::AutoReactions<MostDerived, EventList>::type;
      reaction_result = LocalReact(evt, AutoGeneratedReactions{});
    } else {
      reaction_result = LocalReact(evt, reactions{});
    }
